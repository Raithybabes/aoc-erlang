...
cov_range(Y, Sensor) ->
    ...
    if
        Strength =< 0 -> no_coverage;
        true -> {coverage, Sx - Strength, Sx + Strength}
    end.
...
find_gap_y(Y, MaxY, MinX, MaxX, Sensors) -> 
    ...
    if
        GapX =:= no_gap -> find_gap_y(Y + 1, MaxY, MinX, MaxX, Sensors);
        true -> {GapX, Y}
    end.

find_gap_x(X, MaxX, _) when X > MaxX -> no_gap;
find_gap_x(_, _, []) -> no_gap;
find_gap_x(X, MaxX, [{Rmin, Rmax}|Rest]) ->
    if X < Rmin -> X;
    true -> find_gap_x(max(Rmax + 1, X), MaxX, Rest) end.
