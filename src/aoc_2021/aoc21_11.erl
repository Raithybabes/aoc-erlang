...
octopus_flash({X, Y, V, Grid, {Steps, Flashes}}) ->
    if
        V =:= x -> {grid:set(X, Y, 0, Grid), {Steps, Flashes + 1}};
        ?else -> {Grid, {Steps, Flashes}}
    end.
...
finish_when_all_flash({{W, H, _G} = Grid, {Steps, Flashes}}) ->
    IsFinished = rwm:iif((Flashes =:= (W * H)) or (Steps =:= 1000)),
